class_name SubdivConverter
var importer: TopologyDataImporter
var import_mode: int
var subdiv_level: int

func _init(p_import_mode: TopologyDataImporter.ImportMode, p_subdiv_level: int):
	self.importer=TopologyDataImporter.new()
	self.import_mode=p_import_mode
	self.subdiv_level=p_subdiv_level

static func convert_string_to_import_mode(enum_string: String) -> int:
	match enum_string:
