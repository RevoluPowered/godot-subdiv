@tool
class_name SubdivSceneModifierPlugin
...
func _pre_process(scene: Node):
	...
	var import_enum: TopologyDataImporter.ImportMode = SubdivConverter.convert_string_to_import_mode(subdiv_import_option)
	var subdiv_converter= SubdivConverter.new(import_enum, subdiv_level)
	if scene!=null:
		subdiv_converter.convert_importer_mesh_instances_recursively(scene)

static func subdiv_nodes(scene: Node, subdiv_level: int, subdiv_settings : TopologyDataImporter.ImportMode):
	var subdiv_converter= SubdivConverter.new(subdiv_settings, subdiv_level)
	if scene!=null:
		subdiv_converter.convert_importer_mesh_instances_recursively(scene)
